...
public class RecursiveDirWatcher implements Releasable {
    ...
    private final Map<Path, WatchKey> pathWatchKeyMap
            = new HashMap<>();
    private WatchService watchService = null;
    private Thread watchThread = null;
    private Timer timer = null;

    private Map<Path, List<WatchEvent<?>>> watchableEventMap =
            new TreeMap<>();

    public static RecursiveDirWatcher create(final Path rootDir, FileChangeListener listener) {
        return new RecursiveDirWatcher(rootDir, new ArrayList<FileChangeListener>() {{ add(listener); }});
    }
