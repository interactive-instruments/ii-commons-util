...
public class TaskPoolRegistry<R> {
	...
	private final ArrayBlockingQueue<Runnable> taksQueue;
		
	private final ConcurrentMap<UUID, TaskWithProgressIndication<R>> tasks =
	...
	public TaskPoolRegistry(final int poolSize, final int maxPoolSize) {
		...
		this.taksQueue = new ArrayBlockingQueue<Runnable>(maxPoolSize);
	 ...
	 */
	public synchronized Future<R> submitTask(final TaskWithProgressIndication<R> task) throws
